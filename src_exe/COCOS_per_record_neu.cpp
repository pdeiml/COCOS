...
void GotPhoton(long long TimeTag, int Channel, int DTime)
{
  ...
  if (RecNum % 10000000 == 0){std::cout << RecNum * 1e-6 << "\t Mio\tTime:\t" << 1e-12 * 250 * inputvector2.back() << " s" << std::endl;}//Ausgabe des Fortschritts
}
...
int main (int argc, char* argv[])
{
    ...
    long int vRecordsInPackage = 2.5e10;

    std::cout << "START COCOS" << std::endl;
    Settings vSetting;
    ...
    TH1* allchannelhistogram[2][2];//Histogram array for [basicpoint-CHN][invesitgationpoint-CHN]
	  allchannelhistogram[0][0] = new TH1I("00-Histogram", "Events Channel 0", nbins, 1.*taubeg/250., 1.*tauend/250.);
	  allchannelhistogram[0][1] = new TH1I("01-Histogram", "Cross-Correlation to Channel 0", nbins, 1.*taubeg/250., 1.*tauend/250.);
	  allchannelhistogram[1][0] = new TH1I("10-Histogram", "Cross-Correlation to Channel 1", nbins, 1.*taubeg/250., 1.*tauend/250.);
	  allchannelhistogram[1][1] = new TH1I("11-Histogram", "Events Channel 1", nbins, 1.*taubeg/250., 1.*tauend/250.);
...
//##################################################################################\\
	...
	//Fuer das Rechnen werden die neuen Variablen der puren Timestamps genommen
	double rtaubeg = vSetting.GetTauBegin()/250 - 0.5;
	double rtauend = vSetting.GetTauEnd()/250 - 0.5;
	int rStartEvalTime = vSetting.GetStartEvalTime()/250;
	int rEndEvalTime = vSetting.GetEndEvalTime()/250;
  	...
  	for(RecNum=vStartRecord*vRecordsInPackage;RecNum<(vStartRecord+1)*vRecordsInPackage;RecNum++)
  	{
		...
		else {fileend = true; goto noprocessmarker;}
    	  }
	...
	noprocessmarker:;
    	...
    	if (eventcounter >= 1e8 || RecNum == (vStartRecord+1)*vRecordsInPackage-1 || fileend == true)//Jezt kommt die ganze Auswertung einer 10^8-Reihe!
      {
        ...
        //Evaluate only if last time is not before start evaluation time
                     ...
                     //###############################################//
                     long long partinputs = inputvector1.size();
                     ...
                     if (tauend > 0)//Fill positive time-range
                     {
                       ...
                       for (long long i=0; i<partinputs-1; i++)
                       {
                         ...
                         // if (vSetting.GetTimeLimitation() == true && inputvector2.at(i) < rStartEvalTime){startlinetotime ++; goto skipevalp;}
                         // else if (startmeastimeout == false){startmeastime = 250 * inputvector2.at(i); startmeastimeout = true;}
                         ccounts[inputvector1.at(i)] ++;
                         // //End evaluation complete if current time is larger than the upper adjusted timelimit:
                         // if (vSetting.GetTimeLimitation() == true && inputvector2.at(i) > rEndEvalTime){endlinetotime = i; if(taubeg>0){goto close;}; goto closep;}
                         ...
                         //   //if (i % 10000000 == 0){std::cout << "\t" << 100.*i/(1.*partinputs) << " %" << std::endl;}
                           ...
                           endmeastime = 250 * inputvector2.at(i);//Can be always done, as it always increases
        ...
        //if (startmeastimeout == false){std::cout << "PPPPL: " << startlinetotime << std::endl; startmeastime = 250 * inputvector2.at(startlinetotime); startmeastimeout = true;}

        //Reset important values for the next round
        eventcounter = 0; startlinetotime = 0;
        inputvector1.clear(); inputvector2.clear();
	if (fileend == true){break;}
	}
  	}
...
//############# Ende Code aus der ptu -> txt - Datei #############//
    ...
    //long long starttime = 250 * inputvector2.at(0);//Works only for startinput = 0
    //long long endtime = 250 * inputvector2.at(inputvector2.size()-1);
    ...
    for (int i=0; i<nbins; i++)
    {
	if (correlationarray[i][0][0][0] < 0)
	{
		correlationarray[i][1][0][0] = allchannelhistogram[0][0]->GetBinContent(nbins - i + 1);
		correlationarray[i][1][0][1] = allchannelhistogram[1][0]->GetBinContent(nbins - i + 1);
		correlationarray[i][1][1][0] = allchannelhistogram[0][1]->GetBinContent(nbins - i + 1);
		correlationarray[i][1][1][1] = allchannelhistogram[1][1]->GetBinContent(nbins - i + 1);
	}
	else if (correlationarray[i][0][0][0] == 0)
	{
		...
		correlationarray[i][1][1][0] = allchannelhistogram[0][1]->GetBinContent(i+1);
	...
	for (int b=1; b<nbins; b++)
      {
        ...
        fileout << correlationarray[b][0][0][1] << "\t" << correlationarray[b][1][0][0] << "\t" << correlationarray[b][1][0][1] << "\t" << correlationarray[b][1][1][0] << "\t" << correlationarray[b][1][1][1] << "\n";
      	//std::cout << b << "   " << correlationarray[b][1][0][0] << "   " << correlationarray[b][1][1][1] << std::endl;
	}
      fileout.close();
    }
