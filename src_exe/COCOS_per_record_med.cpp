...
int main (int argc, char* argv[])
{
	...
	std::cout << "Taubeg     " << taubeg << std::endl;
	std::cout << "Taubeg/250:    " << taubeg/250 << std::endl;
    	...
    	if (eventcounter >= 1e8 || RecNum == (vStartRecord+1)*vRecordsInPackage-1 || fileend == true)//Jezt kommt die ganze Auswertung einer 10^8-Reihe!
      {
        ...
        //Evaluate only if last time is not before start evaluation time
                     ...
                     if (taubeg < 0)//Fill negative time-range - do everything backwards
                     {
                       ...
                       {
                         //Pass ranges that are not within the adjusted evaluation time range:
                         // if (vSetting.GetTimeLimitation() == true && inputvector2.at(i) < rStartEvalTime){goto skipevaln;}
                         ...
                         // if (vSetting.GetTimeLimitation() == true && inputvector2.at(i) > rEndEvalTime){goto close;}
                           ...
                           //if (i % 10000000 == 0){std::cout << "\t" << 100.*i/(1.*partinputs) << " %" << std::endl;}
                             ...
                             while (deltat > rtauend && investigationpoint > 0)//Find start Photon, only for tauend < 0
                             {
                                 investigationpoint --;
                                 deltat = inputvector2.at(investigationpoint) - inputvector2.at(basicpoint);
                             }
                  
                             while (deltat > rtaubeg && investigationpoint > 0)
                             {
                                 //BchannelID = inputvector1.at(basicpoint);
                                 IchannelID = inputvector1.at(investigationpoint);   
                                 allchannelhistogram[BchannelID][IchannelID]->Fill(deltat);
