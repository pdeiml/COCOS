...
void Calibrate (){
    ...
    for (int imeas=0; imeas<nmeas; imeas++)
    {
    	for (int bas=0; bas<2; bas++)
    	{
    		for (int inv=0; inv<2; inv++)
    		{
                ...
                if (g2dist[imeas][bas][inv]->GetMaximum() > g2distmax[bas][inv] && imeas != calibrationbasis)
                {
    ...
    for (int bas=0; bas<2; bas++)
    {
        for (int inv=0; inv<2; inv++)
        {
            ...
            g2dist[0][bas][inv]->GetYaxis()->SetRangeUser(0, g2distmax[bas][inv]+2);
            g2dist[0][bas][inv]->Draw();
            g2dist[0][bas][inv]->SetTitle("g_{2} distribution histogram");
            g2dist[0][bas][inv]->GetXaxis()->SetTitle("#tau [ps]");
            g2dist[0][bas][inv]->GetYaxis()->SetTitle("Number of events");
