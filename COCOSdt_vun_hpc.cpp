...
//g++ -o COCOSdt_vun_hpc COCOSdt_vun_hpc.cpp `root-config --cflags --glibs --ldflags` -L/usr/local/lib/root
...
//vectors with reduced entries
std::vector<long long> inputvector0r;
std::vector<long long> inputvector1r;
std::vector<long long> inputvector2r;
std::vector<long long> inputvector3r;
...
int main (int argc, char* argv[])
{
    ...
    dtdiff         =  settingsvector.at(6);
    calibrationmode=  settingsvector.at(7);
    ...
    if (calibrationmode == 1 || calibrationmode == 2)
    {
      ...
      if (forcalibfilename.size() < 10){std::cout << "Could not find calibration file. Please make sure to give a correct name!" << std::endl; exit(1);}
      calibfilename  =  forcalibfilename.at(9);
      settingsfile.close();
    }
    ...
    std::cout << "\tIsolation time (ps):\t" << dtdiff << std::endl;
...
//############# Jetzt wieder jede Menge Code aus dem ptu -> txt - file #############//
    ...
    long long partinputs;
    long long ichn_p, ichn_n;
    long long pdiff, ndiff;
    ...
    //##################################################################################\\
  	...
  	for(RecNum=0;RecNum<NumRecords;RecNum++)
  	{
        ...
        if (inputvector3.back() >= startevaltime)
        {
                     ...
                     //###############################################//
                     partinputs = inputvector0.size();
                     //Only keep 'isolated' events
                     for (long long i=1; i<(partinputs-1); i++)
                     {
                        rechn = inputvector1.at(i);
                        //Look for previous event in channel
                        ichn_p = i-1;
                        while (inputvector1.at(ichn_p) != rechn && ichn_p > 0)
                        {
                          ichn_p --;
                        }
                        pdiff = inputvector3.at(i) - inputvector3.at(ichn_p);

                        //Look for next event in channel
                        ichn_n = i+1;
                        while (inputvector1.at(ichn_n) != rechn && ichn_n < (partinputs-1))
                        {
                          ichn_n ++;
                        }
                        ndiff = inputvector3.at(ichn_n) - inputvector3.at(i);

                        if (pdiff > dtdiff && ndiff > dtdiff && ichn_p > 0 && ichn_n < (partinputs-1))
                        {
                          //Fill new vector
                          inputvector0r.push_back(inputvector0.at(i));
                          inputvector1r.push_back(inputvector1.at(i));
                          inputvector2r.push_back(inputvector2.at(i));
                          inputvector3r.push_back(inputvector3.at(i));
                        }
                     }


                     partinputs = inputvector0r.size();              
                     if (tauend > 0)//Fill positive time-range
                     {
                       ...
                       for (long long i=0; i<partinputs-1; i++)
                       {
                         ...
                         if (timelimitation == true && inputvector3r.at(i) < startevaltime){startlinetotime ++; goto skipevalp;}
                         else if (startmeastimeout == false){startmeastime = inputvector3r.at(i); startmeastimeout = true;}
                         ccounts[inputvector1r.at(i)] ++;
                         //End evaluation complete if current time is larger than the upper adjusted timelimit:
                         if (timelimitation == true && inputvector3r.at(i) > endevaltime){endlinetotime = i; if(taubeg>0){goto close;}; goto closep;}
              
                           //if (i % 10000000 == 0){std::cout << "\t" << 100.*i/(1.*partinputs) << " %" << std::endl;}
                         
                           basicpoint = i;
                           investigationpoint = i+1;
                           deltat = inputvector3r.at(investigationpoint) - inputvector3r.at(basicpoint);
                           BchannelID = inputvector1r.at(basicpoint);
                           //Fill timestamphistogram (only during positive time range)
                           timestamphistogram[BchannelID]->Fill((inputvector3r.at(basicpoint)/250)%8);
                           ...
                           while (deltat < tauend && investigationpoint < partinputs-1)
                           {
                              ...
                              IchannelID = inputvector1r.at(investigationpoint);
                           ...
                           endmeastime = inputvector3r.at(i);//Can be always done, as it always increases
                     ...
                     if (taubeg < 0)//Fill negative time-range - do everything backwards
                     {
                       ...
                       {
                         //Pass ranges that are not within the adjusted evaluation time range:
                         if (timelimitation == true && inputvector3r.at(i) < startevaltime){goto skipevaln;}
                         if (tauend <= 0){ccounts[inputvector1r.at(i)] ++;}//Count events as well when only negative time range
                         //End evaluation completely if actual time is larger than the upper adjusted timelimit:
                         if (timelimitation == true && inputvector3r.at(i) > endevaltime){goto close;}
        ...
        inputvector0.clear(); inputvector1.clear(); inputvector2.clear(); inputvector3.clear();
    		inputvector0r.clear(); inputvector1r.clear(); inputvector2r.clear(); inputvector3r.clear();
    	}
  	}
